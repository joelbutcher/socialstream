...
test('users can register', function (): void {
    ...
    Session::put('socialstream.previous_url', '/register');
...
test('existing users can login', function (): void {
    ...
    Session::put('socialstream.previous_url', '/login');
...
it('can render the prompt page', function () {
    Socialstream::promptOAuthLinkUsing(fn (Provider $provider) => view('socialstream::oauth.test-prompt', ['provider' => $provider]));
    ...
    expect(get('http://localhost/oauth/confirm?provider=github'))
...
it('denies an attempt to link an account', function () {
    Route::get('/test', fn () => 'test')->name('linked-accounts');
    ...
    post('http://localhost/oauth/confirm', data: [
        'provider' => 'github',
        'result' => 'deny',
        'password' => 'password',
    ])
        ->assertRedirect('/test')
        ->assertSessionHas('socialstream.error', 'Failed to link GitHub account. User denied the request.');
});
